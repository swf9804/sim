...
import com.springboot.framework.config.SmsConfig;
...
public class HwSendSmsUtil {
    ...
    public void sendMsg(String phone, String params) throws Exception {
        ...
        String url = smsConfig.getUrl(); //APP接入地址+接口访问URI
        String appKey = smsConfig.getAppKey(); //APP_Key
        String appSecret = smsConfig.getAppSecret(); //APP_Secret
        String sender = smsConfig.getSender(); //国内短信签名通道号或国际/港澳台短信通道号
        String templateId = smsConfig.getTemplateId(); //模板ID

        //条件必填,国内短信关注,当templateId指定的模板类型为通用模板时生效且必填,必须是已审核通过的,与模板类型一致的签名名称
        //国际/港澳台短信不用关注该参数
        String signature = smsConfig.getSignature(); //签名名称
         ...
         */
        String templateParas = "[\""+ params +"\"]"; //模板变量
